...
4. Discover templates before creating a project. Do not guess `template-id`; parse `project-template fields` as either array or object data:
...
8. If archive succeeds with 2xx but the upstream body is empty, the CLI still returns success data with `project_id` and `archived:true`. If archive fails, run read-only `project get` when possible to check the current `logicalStatus`, then report the project ID and the structured `.error` object. Do not attempt hard delete as a fallback, and do not use `raw request` for write fallback.
